...
(function($) {
	var supportedCSS, styles = document.getElementsByTagName("head")[0].style, toCheck = "transformProperty WebkitTransform OTransform msTransform MozTransform"
			.split(" ");
	...
	Wilq32.PhotoEffect = (function() {
		...
		if (supportedCSS) {
			...
			return function(img, parameters) {
				// Make sure that class and id are also copied - just in case
				// you would like to refeer to an newly created object
				this._img = img;
	...
	Wilq32.PhotoEffect.prototype = {
		_setupParameters : function(parameters) {
			...
			this._parameters.easing = parameters.easing
					|| this._parameters.easing || function(x, t, b, c, d) {
			...
			this._parameters.callback = parameters.callback
					|| this._parameters.callback || function() {
					};
		...
		_Loader : (function() {
			if (IE)
				return function(parameters) {
					...
					this._vimage.style.position = "absolute"; // FIXES IE
																// PROBLEM - its
																// only rendered
																// if its on
																// absolute
					...
					/*
					 * Group minifying a small 1px precision problem when
					 * rotating object
					 */
					...
					this._container.setAttribute('coordsize', width - 1 + ','
							+ (height - 1)); // This -1, -1 trying to fix
												// ugly problem with small
												// displacement on IE
					this._container.appendChild(this._vimage);

					this._rootObj.appendChild(this._container);
					this._rootObj.style.position = "relative"; // FIXES IE
																// PROBLEM
				...
				return function(parameters) {
					...
					var _widthMax = Math.sqrt((this._height) * (this._height)
							+ (this._width) * (this._width));

					this._widthAdd = _widthMax - this._width;
					this._heightAdd = _widthMax - this._height; // widthMax
																// because
																// maxWidth=maxHeight
					this._widthAddHalf = this._widthAdd / 2; // used for
																// optimisation
					this._heightAddHalf = this._heightAdd / 2;// used for
																// optimisation
					...
					this._aspectW = ((parseInt(this._img.style.width, 10)) || this._width)
							/ this._img.width;
					this._aspectH = ((parseInt(this._img.style.height, 10)) || this._height)
							/ this._img.height;
		...
		_rotate : (function() {
			...
			if (IE)
				...
				return function(angle) {
					...
					// function is translate, then rotate, then translate,
					// translate..
					this._cnv
							.translate(this._widthAddHalf, this._heightAddHalf); // at
																					// least
																					// center
																					// image
																					// on
					...
					this._cnv.translate(-this._widthHalf, -this._heightHalf); // move
																				...
																				// its
																				// center
					this._cnv.scale(this._aspectW, this._aspectH); // SCALE -
																	// if needed
																	// ;)
					this._cnv.drawImage(this._img, 0, 0); // First - we draw
															// image
				}
