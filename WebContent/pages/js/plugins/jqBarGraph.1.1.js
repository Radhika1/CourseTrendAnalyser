...
//Added by Brandon Moore
 * @param maxValue: 120 //the highest y value you want visible on the graph
...
(function($) {
	...
	max = function(ar){
		if (arr.maxValue != undefined && !isNaN(arr.maxValue)) {
			return arr.maxValue;
		}else {
			maxvalue = 0;
			for(var val in ar){
		...
		colPosition = 0; // for printing colors on simple bar graph
 		...
 		for(var val in data){
 			...
 			if (valueData instanceof Array) 
 				value = Math.round(sum(valueData));
 			else
 				value = valueData;
			...
			unique = val+el.id; //unique identifier
 			...
 			out  = "<div class='graphField"+el.id+" graphField' id='graphField"+unique+"' style='position: absolute'>";
 			out += "<div class='graphValue"+el.id+" graphValue' id='graphValue"+unique+"'>"+prefix+value+postfix+"</div>";
 			
 			out += "<div class='graphBar"+el.id+" graphBar' id='graphFieldBar"+unique+"' style='background-color:"+color+";position: relative; overflow: hidden;'></div>";

			// if there is no legend or exist legends display lbl at the bottom
 			if(!arr.legend || arr.legends)
 				out += "<div class='graphLabel"+el.id+ " graphLabel' id='graphLabel"+unique+"'><div class='graph-label-outer'><span class='graph-label-text'>"+lbl+"</span></div></div>";
 			out += "</div>";
 			
			//So that text overflows evenly
			$('.graph-label-outer').css({
			    'display': 'block',
			    'position': 'relative'});
		
			$('.graph-label-text').css({
				'margin':'0 -75px'});
			...
			//Store the hieght of the bottom label so we can put gridlines behind the graph
			if (typeof labelHeight === "undefined") {
				labelHeight = $('.graphLabel'+el.id).outerHeight(true);	
			}
			
			
			
			//changed margin so that the far right and far left are the same
			...
			// //The last bar needs to have a margin on the right that equals the margin on the left
			// 						if (data[val] == data[data.length-1]) {
			// 							$('#graphField'+unique).css({ 
			// 				 				'left': (fieldWidth)*val, 
			// 				 				'width': fieldWidth-space*2, 
			// 				 				'margin-left': space,
			// 								'margin-right': space});
			// 						} else {
			// 							$('#graphField'+unique).css({ 
			...
			// 						}
 			...
 			if(valueData instanceof Array){
				...
				if(arr.type=="multi"){
 				...
 				for (i in valueData){
 					heig = Math.round(totalHeightBar*valueData[i]/maxe);
 					wid = parseInt((fieldWidth-space)/valueData.length);
 					sv = ''; // show values
 					fs = 0; // font size
 					if (arr.showValues){
						//Keep the values from overlapping
						if(valueData[i] >=10) {
	 						sv = arr.prefix+valueData[i]+arr.postfix;
						}
 						fs = 12; // font-size is 0 if showValues = false
 					}
 					o = "<div class='subBars"+el.id+" sub-bar' style='height:"+heig+"px; background-color: "+arr.colors[i]+"; left:"+wid*i+"px; color:"+arr.showValuesColor+"; font-size:"+fs+"px' >"+sv+"</div>";
...
//			//Add click handlers
//			$('#graphFieldBar'+unique).bind('click',{label:lbl},function(event){
//				var data = [el.id, event.data.label];
//				handleBarClick(data);
//			});
//			
//			$('#graphValue'+unique).bind('click',{label:lbl},function(event){
//				var data = [el.id, event.data.label];
//				handleBarClick(data);
//			});
		...
		//Gridlines		
		if (arr.lines) {
			var lineInterval = arr.lineInterval;
			var numLines = $(el).height()/lineInterval;
			var p = $('.graphBar').position();
			var currentHeight = 0;
			var gridlineWidth = $(el).width();
			for(i=1;i<=Math.round(numLines);i++){
				$(el).prepend("<div class='gridline' style='width:"+gridlineWidth+"px; position:absolute; bottom: " + currentHeight +"px; margin-bottom:" + labelHeight + "px'></div>");
				currentHeight += lineInterval;
			}
		}
...
//	function handleGraphClick(clickEvent) {
//		var test = clickEvent;
//		var anotherTest = $(this);
//		var yetAnotherTest = $this;
//	}
